...
public:
		...
		: Layer("Example"), m_Camera(-1.6f, 1.6f, -0.9f, 0.9f), m_CameraPosition(0.0f)
	{
		...
		m_Shader = BrickEngine::Shader::Create("VertexPosColor", vertexSrc, fragmentSrc);
		...
		m_FlatColorShader = BrickEngine::Shader::Create("FlatColor", flatColorShaderVertexSrc, flatColorShaderFragmentSrc);
		...
		std::dynamic_pointer_cast<BrickEngine::OpenGLShader>(textureShader)->Bind();
		std::dynamic_pointer_cast<BrickEngine::OpenGLShader>(textureShader)->UploadUniformInt("u_Texture", 0);
	}
	...
	virtual void OnUpdate(BrickEngine::Timestep ts) override
	{
		...
		m_Texture->Bind();
		BrickEngine::Renderer::Submit(textureShader, m_SquareVA, glm::scale(glm::mat4(1.0f), glm::vec3(1.5f)));
